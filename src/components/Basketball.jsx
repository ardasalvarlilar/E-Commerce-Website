...
import { StyledGridForPages, StyledGridDivElementImg, StyledGridDivElement, StyledGridDivDesc, AddToCartBtn} from './StyledComponents';
...
const Lifestyle = ({addToCart}) => {
  ...
  useEffect(() => {
    const lifestyleItems = data.filter(item => item.category === 'basketball');
  ...
  return (
    ...
    <div className='px-24 py-0 bg-white'>
      <div className='flex justify-between py-5 px-14'>
        <div>
          <h1 className='text-2xl text-center'>BROWSE BASKETBALL SHOES</h1>
        </div>
        <div>
          ...
          return (
            <StyledGridDivElement className='grid-item' key={i} style={{ backgroundColor: colorArray2[e.id] }}>
              ...
              <StyledGridDivDesc>
                ...
                <div>
                  <AddToCartBtn id={e.id} onClick={addToCart}
